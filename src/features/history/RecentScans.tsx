    ...
    <AppLayout title="Scan History" showBack>
      <div className="space-y-6 pb-12">
        ...
        <section className="grid grid-cols-2 gap-4">
          {/* Discovered Specs */}
          <div className="glass-card-light dark:glass-card p-5 rounded-2xl flex flex-col justify-between border border-slate-100 dark:border-white/10">
            <div>
              <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-1">
                Species Found
              </p>
              <h2 className="text-3xl font-black text-cyan-600 dark:text-cyan-400">
          ...
          <div className="glass-card-light dark:glass-card p-5 rounded-2xl flex flex-col justify-between border border-slate-100 dark:border-white/10">
            ...
            <div className="mt-3 flex items-center text-cyan-600 dark:text-cyan-400 text-xs font-semibold">
            ...
            <button
              ...
              className={`px-4.5 py-2 rounded-full font-sans text-xs font-bold whitespace-nowrap transition-all duration-200 cursor-pointer ${
                activeFilter === btn.key
                  ...
                  : 'bg-slate-200/50 dark:bg-white/15 text-slate-700 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-white/30'
              }`}
            >
              {btn.label}
            </button>
        ...
        <section className="relative pl-6">
          ...
          <div className="absolute left-2 top-4 bottom-0 w-0.5 bg-cyan-600/20 dark:bg-cyan-600/40 pointer-events-none rounded" />
            ...
            <div className="space-y-6">
              {filteredHistory.map((item) => (
                <div key={item.id} className="relative group">
                  ...
                  <div className="absolute -left-6 top-1/2 -translate-x-1/2 -translate-y-1/2 w-4 h-4 rounded-full border-2 border-cyan-400 bg-white dark:bg-slate-900 z-10" />

                  <div className="glass-card-light dark:glass-card p-5 rounded-2xl flex gap-5 hover:shadow-lg transition-all duration-300 border border-slate-100 dark:border-white/10 relative">
                    {/* Photo Thumbnail */}
                    <div 
                    ...
                    <div className="flex-grow min-w-0 flex flex-col justify-between">
                      <div>
                        <div className="flex justify-between items-start mb-1 gap-1">
                          ...
                          <span className="text-[9px] font-black tracking-tight text-cyan-600 dark:text-cyan-400 bg-cyan-50 dark:bg-cyan-950/50 px-1.5 py-0.5 rounded flex-shrink-0">
                            {item.confidence.toFixed(1)}% AI
                          </span>
                        </div>
                        <p className="text-xs italic text-slate-500 truncate mb-2">
                          {item.scientificName}
                        </p>
                      </div>
