  ...
  return (
    <AppLayout title="Analytics" showBack>
      <div className="space-y-4">
        <div className="rounded-[28px] border border-[#0A66FF]/10 bg-white/90 p-5 shadow-[0_10px_30px_rgba(10,102,255,0.08)]">
          <div className="flex items-center gap-2 text-[#0A66FF]">
          ...
          {stats.map((item) => {
            ...
            return (
              <div key={item.label} className="rounded-[24px] border border-slate-100 bg-white p-4 shadow-sm">
                ...
                <p className="mt-3 text-2xl font-black text-[#1a2a3a]">{item.value}</p>
        ...
        <div className="rounded-[28px] border border-slate-100 bg-white p-5 shadow-sm">
          <p className="text-sm font-semibold text-[#1a2a3a]">Quick insight</p>
          <p className="mt-2 text-sm text-slate-600">
            {totalScans === 0
